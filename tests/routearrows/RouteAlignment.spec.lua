...
local printed = {}
-- Un MitzuMPlus FALSO hace de core (con EventBus y RouteManager REALES).
-- MitzuRouteArrows lo lee por la API publica REAL y la fachada REAL.
MitzuMPlus = { AdaptiveRoute = {}, db = {} }
_G.MitzuMPlus = MitzuMPlus
function LibStub() return { GetAddon = function() return MitzuMPlus end } end
MitzuRouteArrowsDB = {}

dofile("MitzuMPlus/modules/EventBus.lua")
...
dofile("MitzuMPlus/data/Routes/Season2/RubyLifePools.lua")
...
MitzuRouteArrows.Print = function(_, m) printed[#printed + 1] = m end

function MitzuRouteArrows:VisibleNameplates()
    local out = {}
...
dofile("MitzuMPlus/modules/RouteManager.lua")
dofile("MitzuRouteArrows/Evidence/EventCastEvidence.lua")
dofile("MitzuRouteArrows/Modules/NameplateGenerations.lua")
dofile("MitzuRouteArrows/Modules/ArrowDemoTelemetry.lua")
dofile("MitzuRouteArrows/Modules/ArrowDemo.lua")
dofile("MitzuRouteArrows/Alignment/RouteSignature.lua")
dofile("MitzuRouteArrows/Alignment/ExecutionEpisodeTracker.lua")
dofile("MitzuRouteArrows/Alignment/RoutePullCandidateScorer.lua")
dofile("MitzuRouteArrows/Alignment/RouteAlignment.lua")
...
-- ═══════════════════════════════════════════════════════════════════════════
-- A · FIRMA DE LA RUTA (RouteSignature, sobre la ruta leida por la API)
-- ═══════════════════════════════════════════════════════════════════════════
...
test("A5 la firma se calcula sobre la ruta leida por la API, sin MDT", function()
    -- Antes era RouteManager:GetPullSignature, dentro del core. Tras la
    -- separacion el core no sabe nada de firmas: esta capa pide la ruta por
    -- MitzuMPlusAPI y calcula la firma ella misma.
    ACTIVE_ROUTE = RUBY
    local ruta = AR.Host.RouteProgress:GetRoute()
    truthy(ruta, "la ruta llega por la API")
    truthy(ruta ~= RUBY, "y llega como COPIA, no la tabla del core")
    local sig = RS:ForPull(ruta, 9)
    equal(sig.mobCount, 18, "pull 9 de Ruby")
    equal(sig.pull, 9, "indice")
    equal(RM.GetPullSignature, nil, "el core ya no expone firmas de pull")
    equal(RS:ForPull(nil, 1).state, "UNKNOWN", "sin ruta no hay firma")
end)
...
local function leer(path)
    ...
    -- Y el NOMBRE DEL ADDON (MitzuRouteArrows, en `_G.MitzuRouteArrows`)
    -- contiene la palabra RouteArrows: se quita tambien, porque lo que se
    -- prohibe es usar el MODULO de flechas, no pertenecer a este addon.
    s = s:gsub("%-%-[^\n]*", "")
    return (s:gsub("MitzuRouteArrows", "MRA_ADDON"))
end
...
test("G1 ningun modulo de la fase 4 escribe en RouteProgress", function()
    for _, nombre in ipairs({ "RouteSignature", "ExecutionEpisodeTracker",
        ...
        local src = leer("MitzuRouteArrows/Alignment/" .. nombre .. ".lua")
...
test("G3 el scorer es puro: sin estado propio ni acceso al cliente", function()
    local src = leer("MitzuRouteArrows/Alignment/RoutePullCandidateScorer.lua")
...
test("G5 el TOC de MitzuRouteArrows carga los cuatro modulos y en orden", function()
    local toc = leer("MitzuRouteArrows/MitzuRouteArrows.toc")
    local orden = {
        [[Modules\ArrowDemo.lua]],
        [[Alignment\RouteSignature.lua]],
        [[Alignment\ExecutionEpisodeTracker.lua]],
        [[Alignment\RoutePullCandidateScorer.lua]],
        [[Alignment\RouteAlignment.lua]],
    }
    local anterior = 0
...
test("G6 ArrowDemo alimenta la capa, y de forma opcional", function()
    local src = leer("MitzuRouteArrows/Modules/ArrowDemo.lua")
    ...
    local ali = leer("MitzuRouteArrows/Alignment/RouteAlignment.lua")
    local llamadas = 0
