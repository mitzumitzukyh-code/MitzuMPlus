...
_G.MitzuMPlus = { AdaptiveRoute = {}, MDTEnemyData = {} }
...
AR.PullNavigator = { GetCurrentPull = function() return 1 end }
...
dofile("modules/AdaptiveRoute/LiveEnemyResolver.lua")
...
test("ambiguous npcID never produces automatic arrow", function()
    ...
    marked = 0
    local decision = MitzuMPlus.GuidanceEngine:Evaluate("plateArrow")
    equal(decision.state, "AMBIGUOUS", "arrow decision state")
    equal(decision.shouldMark, false, "arrow decision mark")
    equal(MitzuMPlus.RouteArrowPresenter:Apply("plateArrow", decision), false,
        "presenter result")
