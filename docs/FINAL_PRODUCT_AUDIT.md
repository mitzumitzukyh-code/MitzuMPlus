...
Version inspected: `7.14.0-rc1` (the product was later renumbered to the `1.0.0` series; current `1.0.0-beta.1`). No version bump, commit, push, merge, tag or release.
...
| `SpecDatabase` | Spec/role metadata scaffold | None (never loaded by the TOC) | REMOVED | Deleted before 1.0.0-beta.1: no consumers, dead code |
