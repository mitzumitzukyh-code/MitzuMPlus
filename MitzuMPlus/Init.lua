...
function MitzuMPlus:CreateMainWindow()
    ...
    settingsIcon:SetTexture("Interface\\AddOns\\MitzuMPlus\\Media\\Icons\\2_settings")
    ...
    closeIcon:SetTexture("Interface\\AddOns\\MitzuMPlus\\Media\\Icons\\3_close")
    ...
    elseif cmd == "rutamdt" or cmd == "cargarruta" then
        ...
        if not (AR and AR.ImportFromOpenMDT) then
            ...
            if not ok then
                ...
                if info.mapped then
                    self:Print("  npcIDs mapeados: el delta de tropas por pack funcionara.")
                else
                    self:Print("  |cFFff9922Sin npcIDs|r (" .. tostring(info.npcWhy or "MDT no dio datos de enemigos") ..
                               "): habra delta y skips calculados por pull, sin detalle por mob.")
                end
            end
        end
