...
function AR:StopForRun(save)
    ...
    -- Las flechas y sus anotaciones ya no son del core. MitzuRouteArrows se
    -- limpia solo al recibir KEY_COMPLETED / KEY_RESET / RUN_TEARDOWN por la
    -- API publica; el core no necesita saber que existe.
    if self.PullNavigator then self.PullNavigator:Reset() end
    if save and PM then
        local st = PM:Settings()
...
function AR:SyncReport(quiet)
    ...
    -- 3) Las flechas de ruta ya no forman parte de este parte: viven en el
    --    addon MitzuRouteArrows, que tiene su propio diagnostico (/mra status).
    --    MitzuMPlus no depende de Threat Plates ni de ningun addon de placas.
    ...
    if profile then
        ...
        if idx and idx.hasCloneData then
            ...
            add("               Reimportala con |cFFf7d470/emp rutastodas|r para tener la identidad de cada pull.")
        end
    else
        allOk = false
