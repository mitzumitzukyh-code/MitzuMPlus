...
function DataStructure:Build(profile)
    ...
    local idx = {
        ...
        cloneCount   = 0,
        hasCloneData = false,
        -- Metadatos estaticos de MDT, por identidad de ruta. Son contexto
        -- geometrico; no se usan para resolver unidades ni avanzar pulls.
        physicalByClone = {},
        clonesWithPhysicalMetadata = 0,
        clonesWithPhysicalGroup = 0,
    }
                    ...
                    if type(en.clones) == "table" then
                        for _, c in ipairs(en.clones) do
                            ...
                            if ci then
                                ...
                                local snapshot = MitzuMPlus.MDTPhysicalGroupData
                                local dungeon = snapshot and snapshot[idx.mdtDungeonIdx]
                                local enemyMeta = dungeon and dungeon[ei]
                                local meta = embedded or (enemyMeta and enemyMeta[ci])
                                if type(meta) == "table" then
                                    cloneMetadata[ci] = meta
                                    idx.physicalByClone[ei] = idx.physicalByClone[ei] or {}
                                    idx.physicalByClone[ei][ci] = meta
                                    if isNewClone then
                                        idx.clonesWithPhysicalMetadata =
                                            idx.clonesWithPhysicalMetadata + 1
                                        if tonumber(meta.g) then
                                            idx.clonesWithPhysicalGroup =
                                                idx.clonesWithPhysicalGroup + 1
                                        end
                                    end
                                end
                            end
...
-- Geometria estatica del clon de MDT. Devuelve una referencia de solo lectura.
-- Su presencia no demuestra que ninguna unidad viva sea ese clon.
function DataStructure:PhysicalOfClone(enemyIdx, cloneIdx)
    local idx = self._index
    if not idx then return nil end
    local byEnemy = idx.physicalByClone[tonumber(enemyIdx) or -1]
    return byEnemy and byEnemy[tonumber(cloneIdx) or -1] or nil
end
