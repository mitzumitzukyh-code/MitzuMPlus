...
function RuntimeCapabilities:Probe()
    ...
    -- El anclaje lo sabe RouteArrows, que es quien lo resuelve de verdad.
    do
        local AR = MitzuMPlus.AdaptiveRoute
        local RA = AR and AR.RouteArrows
        local ancla = RA and RA._lastAnchor or nil
        if ancla and ancla ~= "sin resolver" then
            set("nameplateAnchor", AVAILABLE, tostring(ancla))
        elseif RA then
            set("nameplateAnchor", UNKNOWN, "aún no se ha anclado ninguna flecha")
        else
            set("nameplateAnchor", UNAVAILABLE, "RouteArrows no cargado")
        end
    end
