-- ═══════════════════════════════════════════════════════════════════════════
-- MitzuRouteArrows · ArrowDemoTelemetry v1.0  —  EXPERIMENTAL
--
...
-- Topes. Configurables desde aquí; /mra arrowdemo status los enseña.
T.LIMITS = {
...
-- ALMACÉN
--
-- Vive en MitzuRouteArrowsDB.arrowDemo (ver T:Store). Antes de que el
-- SavedVariable exista (o en las pruebas) se usa una tabla en memoria.
-- ─────────────────────────────────────────────────────────────────────────

T._memory = { runs = {} }

-- La telemetria vive en MitzuRouteArrowsDB.arrowDemo, la base de datos de
-- ESTE addon. Antes estaba en la SavedVariable de MitzuMPlus; tras la
-- separacion ese fichero ya no es asunto de este addon.
function T:Store()
    local root = rawget(_G, "MitzuRouteArrowsDB")
    local s
...
function T:Error(where)
    ...
    if s.errorsPrinted == 0 and MRA.Print then
        s.errorsPrinted = 1
        pcall(MRA.Print, MRA,
            "|cFFff9922Arrow Demo: error interno registrado (/mra arrowdemo report).|r")
    end
end
...
if Host.EventBus then
    ...
    Host.EventBus:On("MITZU_PULL_CHANGED", function(n, prev, reason)
        if not T._session then return end
