-- ═══════════════════════════════════════════════════════════════════════════
-- MitzuRouteArrows · ArrowDemo v1.0  —  DEMO VISUAL, NO PRODUCCIÓN
--
...
-- Parámetros de la heurística. Todos visibles en /mra arrowdemo status.
ArrowDemo.PARAMS = {
...
function ArrowDemo:ProductionClaims(token)
    local Pr = MRA.RouteArrowPresenter
...
-- que no es suya (producción, /mra marktarget, /mra arrow).
...
local function visibles()
    if type(MRA.VisibleNameplates) == "function" then
        local ok, l = pcall(MRA.VisibleNameplates, MRA)
...
local function esperados()
    local PE, RP = AR.PackEvidence, Host.RouteProgress
...
local function imprimir(msg)
    if MRA.Print then pcall(MRA.Print, MRA, msg) end
end
...
function ArrowDemo:SetEnabled(on, silent)
    ...
    if on then
        ...
        self:_StartTicker()
        imprimir(silent and "|cFFff9922ARROW DEMO sigue activo|r (se desactiva con /mra arrowdemo off)."
...
function ArrowDemo:OnKeyEnded(status)
    ...
    if tel and tel:IsRecording() then
        ...
        imprimir("|cFFe8b84aArrow Demo:|r run guardada (" .. tostring(status) ..
                 "). Resumen: |cFFf7d470/mra arrowdemo report|r · copiar: |cFFf7d470/mra arrowdemo export|r")
    end
    self:_ResetState()
end
