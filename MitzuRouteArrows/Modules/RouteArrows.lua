-- ═══════════════════════════════════════════════════════════════════════════
-- MitzuRouteArrows · RouteArrows v1.0
--
...
-- Los ajustes viven en MitzuRouteArrowsDB (MRA:Settings), y este módulo tiene que
...
-- en DEFAULTS no basta, porque la fusión de ajustes solo rellena
...
-- Los ajustes viven en la base de datos PROPIA de este addon
-- (MitzuRouteArrowsDB), no en la de MitzuMPlus.
local function settings()
    local st = MRA:Settings()
...
local function dbg(fmt, ...)
    ...
    if not MRA.Print then return end
    MRA:Print("|cFF8fd3ff[RouteArrows]|r " .. string.format(fmt, ...))
end
...
-- /mra arrow debug dice cuál de las dos vías está en uso.
-- ─────────────────────────────────────────────────────────────────────────
