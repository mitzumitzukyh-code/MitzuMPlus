-- ═══════════════════════════════════════════════════════════════════════════
-- MitzuRouteArrows · LiveEnemyResolver v1.0  —  SPIKE, NO PRODUCCIÓN
--
...
function LiveEnemyResolver:BuildCurrentPullIndex(pullIndex)
    local DS = Host.DataStructure
    local PN = Host.PullNavigator
    ...
    local snapshot = dungeonIdx and Host.MDTEnemyData
                     and Host.MDTEnemyData[dungeonIdx] or nil
...
--
-- Resolve() devuelve la observacion ENTERA porque /mra probe y el spike la
